...
namespace StateMachine
{
    ...
    [System.Serializable]
    public struct SM_StateMachineResult
    {
        public EStateMachineCompletionType CompletionType;
        public SM_State FinalState;
    ...
    public class SM_State : ScriptableObject
    {
        ...
        public virtual SM_StateMachineResult RunState(object refObject, List<InputUnit> dataSource, int dataIndex = 0, int remainingSteps = -1)
        {
            ...
            SM_StateMachineResult SMR;
            SMR.FinalState = this;
            SMR.DataIndex = dataIndex;
            SMR.CompletionType = bMustEndNow ? CompletionType : EStateMachineCompletionType.OutOfSteps;
            return SMR;
        }
        protected virtual SM_StateMachineResult LoopState(object refObject, List<InputUnit> dataSource, int dataIndex, int remainingSteps)
        {
        ...
        public List<SM_BranchBase> InstancedBranches; // Branches to other states. These are in priority order, so first successful branch will be taken.
        public List<SM_BranchBase> SharedBranches; // Branches to other states. These are in priority order, so first successful branch will be taken.
    }
}